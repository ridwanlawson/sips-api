...
class AttendanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Antisipasi constraint: 1 karyawan hanya boleh 1 absensi per hari
            if (
                Attendance::where('KODE_KARYAWAN', $request->kode_karyawan)
                    ->whereRaw(
                        "TRUNC(TANGGAL) = TO_DATE(?, 'YYYY-MM-DD')",
                        [$request->tanggal],
                    )
                    ->exists()
            ) {
                return response()->json(
                    [
                        'success' => false,
                        'message' => 'Karyawan sudah melakukan absensi pada tanggal tersebut.',
                    ],
                    400,
                );
            }
            ...
            $datas = Attendance::create([
                'TANGGAL' => $request->tanggal,
                'KODE_KARYAWAN_MANDOR' => $request->kode_karyawan_mandor ?? $idkode_karyawan_mandor,
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('ba_deleted')) {
                ...
                $this->trackUploadedFile($baDeletedPath);
            }

            DB::transaction(function () use ($datas, $baDeletedPath) {
