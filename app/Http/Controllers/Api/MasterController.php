...
class MasterController extends Controller
{
    ...
    {
        $query = Employee::whereNull("DATETERMINATE")->select(
            "FCCODE",
            "FCNAME",
            "GANGCODE",
            "SECTIONNAME",
    ...
    {
        $query = Vehicle::where("ACTIVATION", "=", "Y")->select(
            "FCCODE",
            "FCNAME",
            "VEHICLEGROUPCODE",
            "DATECREATED",
    ...
    {
        $users = User::where("STATUS", "=", "Y")->select("*");
    ...
    {
        $query = BusinessUnit::where("ISACTIVE", "=", "TRUE")->select(
            "FCCODE",
            "FCNAME",
            "FCCOMPANYCODE",
            "FCCOMPANYNAME",
    ...
    /**
     * Data Section/Afdeling/Division dari SIPS Production
     *
     * Digunakan untuk inisialisasi data Section/Afdeling/Division pada SIPS Mobile.
     * Tetapi jika ingin melakukan filter pada data yang dipanggil, buatlah parameter pada Url berdasarkan _**Query Parameter**_
     *
     * @queryParam fccode string Optional. Filter Section/Afdeling/Division berdasarkan kode. Example: AFD-01
     * @queryParam fcba string Optional. Filter Section/Afdeling/Division berdasarkan bisnis unit. Example: MTE
     */
    public function section(Request $request)
    {
        $query = Section::where("ACTIVATION", "=", "Y")->select(
            "FCCODE",
            "FCNAME",
            "FCBA",
        );
        ...
        // Eksekusi query dan dapatkan data
        $query->orderBy("ORDERDATA", "asc");
        $datas = $query->get();

        return new AllResource(
            true,
            "List Data Section/Afdeling/Division",
            $datas,
        );
    }

    /**
     * Data Gang dari SIPS Production
     *
     * Digunakan untuk inisialisasi data Gang pada SIPS Mobile.
     * Tetapi jika ingin melakukan filter pada data yang dipanggil, buatlah parameter pada Url berdasarkan _**Query Parameter**_
     *
     * @queryParam fccode string Optional. Filter Gang berdasarkan kode. Example: PN011
     * @queryParam afdeling string Optional. Filter Gang berdasarkan section/afdeling/division. Example: AFD-01
     * @queryParam fcba string Optional. Filter Gang berdasarkan bisnis unit. Example: MTE
     */
    public function gang(Request $request)
    {
        ...
        if ($request->has("afdeling")) {
            $query->where("REMARKS", "like", "%" . $request->afdeling . "%");
        }
